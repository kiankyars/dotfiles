...
2. Read each MOV with `ffprobe`, using `com.apple.quicktime.creationdate` as its real start and `creation_time` as its real end. Merge overlapping or touching intervals and sum them for the human duration; exclude gaps between sessions.
...
5. Delete the source UUIDs with `uv run scripts/delete_photos_assets.py --uuid UUID ... --confirm-delete --auto-confirm-dialog` and remove the staging exports.
